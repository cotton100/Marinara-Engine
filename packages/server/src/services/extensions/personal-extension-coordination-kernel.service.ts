...
export function createPersonalExtensionCoordinationKernel(
  ...
  const runExtensionRetirementMutation = async <T>(extensionId: string, mutation: (tx: DB) => Promise<T>) => {
    requireIdentifier(extensionId);
    if (typeof mutation !== "function") throw kernelError("invalid-request");
    return profileRestoreAdmissionGate.runExclusive(() =>
      mutexFor(extensionId).runExclusive(async () => {
        let hadCoordinationState = false;
        let result: T;
        try {
          result = await db.transaction(async (tx) => {
            const rows = await tx
            ...
            if (row && row.mode !== "inactive") throw kernelError("coordination-transition-blocked");
            if (row) requireAuthorityClear(row);

            const journals = await tx
              .select()
              .from(personalExtensionOperationJournal)
              .where(eq(personalExtensionOperationJournal.extensionId, extensionId));
            if (journals.some((journal) => journal.phase !== "final")) throw kernelError("operations-active");
            ...
            hadCoordinationState = row !== null || journals.length > 0;
            // Ordinary extensions never required directory-fsync support for
            // uninstall. Require the strict barrier only when retiring durable
            // coordination authority or recovery evidence.
            if (hadCoordinationState) await tx._fileStore.flushStrict();
            return pending;
          });
        } catch (error) {
          if (error instanceof PersonalExtensionCoordinationKernelError) throw error;
        ...
        if (hadCoordinationState) clearRuntimeAuthority(extensionId);
        return result;
      }),
    );
  };
  ...
  const blockAcquireTransition = async (tx: DB, row: PersonalExtensionCoordinationRow, wallMs: number) => {
    const fence = nextFence(row.fence);
    await tx
      .update(personalExtensionCoordination)
      .set({
  ...
  const blockAcquireIfJournalUnresolved = async (tx: DB, row: PersonalExtensionCoordinationRow, wallMs: number) => {
    ...
    const committed = await runStrictMutation(
      ...
      async (tx) => {
        ...
        if (handoff) {
          ...
          if (row.leaseTokenDigest === null || row.holderSessionId === handoff.requester) {
            throw kernelError("handoff-pending");
          }
        ...
        const freshOperation = assertOperationLive(row, input, transitionMonotonicMs, transitionWallMs);
        if (
        ...
        const remainingMs = Math.floor(
          Math.min(deadlineWallMs - transitionWallMs, deadlineMonotonicMs - transitionMonotonicMs),
        );
  ...
  return {
    ...
    runExtensionRetirementMutation,
