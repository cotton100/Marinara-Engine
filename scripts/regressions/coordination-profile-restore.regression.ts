...
async function runStrictRestoreFailureScenario(
  ...
  const assetPath = "avatars/restore-durability-proof.bin";
  const assetOutputPath = join(scenarioDir, "avatars", "restore-durability-proof.bin");
  const assetRollbackPathSuffix = join("rollback", "avatars", "restore-durability-proof.bin");
  const assetRollbackDirectorySuffix = join("rollback", "avatars");
  const liveAsset = Buffer.from("live-profile-asset");
  const backupAsset = Buffer.from("backup-profile-asset");
    ...
    fileDb = await createFileNativeDB({
      fileOperations: {
        writeFile: async (path, content) => {
          if (String(path).includes("app_settings.json.tmp-") && String(content).includes(`${failurePoint}-backup`)) {
        ...
        flushFile: async (path) => {
          ...
          if (
            ...
            String(path).includes("app_settings.json.tmp-") &&
            !failureInjected
          ) {
            failureInjected = true;
            throw expectedFailure;
...
async function runRestoreAuthorityPhaseFailureScenario(phase: "A" | "C") {
  ...
  try {
    fileDb = await createFileNativeDB({
      fileOperations: {
        writeFile: async (path, content) => {
          if (armed && String(path).includes("personal_extension_coordination.json.tmp-")) {
        ...
        flushFile: async (path) => {
          if (
            armed &&
            targetWriteSeen &&
            String(path).includes("personal_extension_coordination.json.tmp-") &&
            !failureInjected
          ) {
            failureInjected = true;
            throw expectedFailure;
...
async function runFinalBarrierConcurrencyScenario() {
  ...
  try {
    fileDb = await createFileNativeDB({
      fileOperations: {
        writeFile: async (path, content) => {
          ...
          if (barrierPaused && String(path).includes("personal_extension_coordination.json.tmp-")) {
    ...
    const durableRowsDuringBarrier = JSON.parse(
      readFileSync(join(scenarioDir, "tables", "personal_extension_coordination.json"), "utf8"),
