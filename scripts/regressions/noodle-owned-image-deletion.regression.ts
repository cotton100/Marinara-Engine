...
import { isNoodleOwnedImageDeletionEnabled } from "../../packages/server/src/config/runtime-config.js";
...
import { noodleRoutes } from "../../packages/server/src/routes/noodle.routes.js";
...
const originalNoodleDeletionFlag = process.env.ENABLE_NOODLE_OWNED_IMAGE_DELETION;
...
try {
  delete process.env.ENABLE_NOODLE_OWNED_IMAGE_DELETION;
  assert.equal(isNoodleOwnedImageDeletionEnabled(), false);
  process.env.ENABLE_NOODLE_OWNED_IMAGE_DELETION = "true";
  assert.equal(isNoodleOwnedImageDeletionEnabled(), true);
  process.env.ENABLE_NOODLE_OWNED_IMAGE_DELETION = "false";
  assert.equal(isNoodleOwnedImageDeletionEnabled(), false);

  const routeApp = Fastify();
  routeApp.decorate("db", db);
  await routeApp.register(noodleRoutes, { prefix: "/api/noodle" });
  const hiddenCapability = await routeApp.inject({ method: "GET", url: "/api/noodle/image-deletion/capabilities" });
  assert.equal(hiddenCapability.statusCode, 404);
  const blockedDelete = await routeApp.inject({ method: "DELETE", url: "/api/noodle/posts/owned/image" });
  assert.equal(blockedDelete.statusCode, 404);
  process.env.ENABLE_NOODLE_OWNED_IMAGE_DELETION = "true";
  const visibleCapability = await routeApp.inject({ method: "GET", url: "/api/noodle/image-deletion/capabilities" });
  assert.equal(visibleCapability.statusCode, 200);
  assert.equal(visibleCapability.json().id, NOODLE_OWNED_IMAGE_DELETION_CAPABILITY.id);
  ...
  const noodlerFixture = await createRandomUserOwnedFixture("noodler-platform", "noodler-platform.png");
  await db
    .update(noodleAccounts)
    .set({ platform: "noodler", visibility: "private" })
    .where(eq(noodleAccounts.id, noodlerFixture.post.authorAccountId));
  await expectCode("ownership-unproven", () =>
    deleteOwnedNoodlePostImage({ db, postId: noodlerFixture.post.id, galleryRoot }),
  );
  assert.equal(existsSync(noodlerFixture.filePath), true);
  const preservedNoodlerRows = await db.select().from(noodlePosts).where(eq(noodlePosts.id, noodlerFixture.post.id));
  assert.equal(preservedNoodlerRows[0]?.imageUrl, noodlerFixture.post.imageUrl);

  const activeClaimFixture = await createRandomUserOwnedFixture("active-claim", "active-claim.png");
  await db
    .update(noodlePosts)
    .set({
      imageClaimToken: "active-claim-token",
      imageClaimLeaseUntil: "2026-08-05T00:01:00.000Z",
    })
    .where(eq(noodlePosts.id, activeClaimFixture.post.id));
  await expectCode("concurrent-change", () =>
    deleteOwnedNoodlePostImage({
      db,
      postId: activeClaimFixture.post.id,
      galleryRoot,
      at: "2026-08-05T00:00:00.000Z",
    }),
  );
  assert.equal(existsSync(activeClaimFixture.filePath), true);
  const preservedActiveClaimRows = await db
    .select()
    .from(noodlePosts)
    .where(eq(noodlePosts.id, activeClaimFixture.post.id));
  assert.equal(preservedActiveClaimRows[0]?.imageUrl, activeClaimFixture.post.imageUrl);
  assert.equal(preservedActiveClaimRows[0]?.imageClaimToken, "active-claim-token");

  const expiredClaimPost = await createRandomUserPost({
    entityId: "expired-claim",
    filename: "expired-claim.png",
    metadata: {
      imagePendingReview: true,
      imageCrop: { x: 0.1, y: 0.2, width: 0.7, height: 0.6 },
    },
  });
  const expiredClaimPath = join(noodleDir, "expired-claim.png");
  writeFileSync(expiredClaimPath, "expired claim fixture");
  await db
    .update(noodlePosts)
    .set({
      imageClaimToken: "expired-claim-token",
      imageClaimLeaseUntil: "2026-08-04T23:59:00.000Z",
    })
    .where(eq(noodlePosts.id, expiredClaimPost.id));
  const expiredClaimDeleted = await deleteOwnedNoodlePostImage({
    db,
    postId: expiredClaimPost.id,
    galleryRoot,
    at: "2026-08-05T00:00:00.000Z",
  });
  assert.equal(expiredClaimDeleted.status, "deleted");
  assert.equal(existsSync(expiredClaimPath), false);
  const clearedExpiredClaimRows = await db.select().from(noodlePosts).where(eq(noodlePosts.id, expiredClaimPost.id));
  assert.equal(clearedExpiredClaimRows[0]?.imageClaimToken, null);
  assert.equal(clearedExpiredClaimRows[0]?.imageClaimLeaseUntil, null);
  const clearedExpiredMetadata = JSON.parse(clearedExpiredClaimRows[0]?.metadata ?? "{}") as Record<string, unknown>;
  assert.equal("imagePendingReview" in clearedExpiredMetadata, false);
  assert.equal("imageCrop" in clearedExpiredMetadata, false);

  const malformedClaimFixture = await createRandomUserOwnedFixture("malformed-claim", "malformed-claim.png");
  await db
    .update(noodlePosts)
    .set({ imageClaimToken: "malformed-claim-token", imageClaimLeaseUntil: "not-a-date" })
    .where(eq(noodlePosts.id, malformedClaimFixture.post.id));
  await expectCode("concurrent-change", () =>
    deleteOwnedNoodlePostImage({
      db,
      postId: malformedClaimFixture.post.id,
      galleryRoot,
      at: "2026-08-05T00:00:00.000Z",
    }),
  );
  assert.equal(existsSync(malformedClaimFixture.filePath), true);
  ...
  assert.ok(
    await noodle.updateAccountProfile(bannerAccount.id, {
      profile: {
  ...
  const parseGuardFixture = await createRandomUserOwnedFixture("parse-guard-target", "parse-guard.png");
  const parseGuardHolder = await createRandomUserPost({
    entityId: "parse-guard-holder",
    filename: "parse-guard-holder.png",
  });
  await db.update(noodlePosts).set({ metadata: "{" }).where(eq(noodlePosts.id, parseGuardHolder.id));
  await expectSharedReferenceBlocked({
    postId: parseGuardFixture.post.id,
    filePath: parseGuardFixture.filePath,
  });
  await db.update(noodlePosts).set({ metadata: "{}" }).where(eq(noodlePosts.id, parseGuardHolder.id));

  await db.update(noodleAccounts).set({ settings: "{" }).where(eq(noodleAccounts.id, parseGuardHolder.authorAccountId));
  await expectSharedReferenceBlocked({
    postId: parseGuardFixture.post.id,
    filePath: parseGuardFixture.filePath,
  });
  await db
    .update(noodleAccounts)
    .set({ settings: "{}" })
    .where(eq(noodleAccounts.id, parseGuardHolder.authorAccountId));
  ...
  for (const ambiguousLocalUrl of [
    "/api/global-gallery/file/noodle/parse-guard.png",
    "/api/characters/personas/persona-x/gallery/file/noodle/parse-guard.png",
  ]) {
    await db.update(noodlePosts).set({ imageUrl: ambiguousLocalUrl }).where(eq(noodlePosts.id, parseGuardHolder.id));
    await expectSharedReferenceBlocked({
      postId: parseGuardFixture.post.id,
      filePath: parseGuardFixture.filePath,
    });
  ...
  await db
    .update(noodlePosts)
    .set({ imageUrl: "https://example.invalid/unrelated-parse-guard.png" })
    .where(eq(noodlePosts.id, parseGuardHolder.id));
  ...
  if (originalNoodleDeletionFlag === undefined) delete process.env.ENABLE_NOODLE_OWNED_IMAGE_DELETION;
  else process.env.ENABLE_NOODLE_OWNED_IMAGE_DELETION = originalNoodleDeletionFlag;
