...
const assetPath = "avatars/profile-asset-gate.png";
const assetOutputPath = join(scenarioDir, assetPath);
const rollbackPathSuffix = join("rollback", assetPath);
const liveAssetA = pngAsset("asset-A-before-restore");
const concurrentAssetB = pngAsset("asset-B-from-ordinary-writer");
const backupAssetC = pngAsset("asset-C-from-profile-backup");
...
try {
  ...
  await fileDb._fileStore.flushStrict();
  const strictAssetOne = pngAsset("strict-asset-one");
  const strictAssetTwo = pngAsset("strict-asset-two");
  autonomousActivity.recordUserActivity(restoredChatId);
  assert.ok(autonomousActivity.getActivityState(restoredChatId));
